    ...
    sAnimations.registry.reset_password_popup = sAnimations.Class.extend({
        ...
        customerRegistration: function(){
            $("#loginRegisterPopup .oe_signup_form_ept").submit(function(e) {
                ...
                if($('#news_subscribe_signup_form').prop("checked") == true){
                	var is_subscribe_true = '1';
                }
                else if($('#news_subscribe_signup_form').prop("checked") == false){
                	var is_subscribe_true = '0';
                }
                ajax.jsonRpc('/web/signup_custom', 'call', {'login':email,'name':name,'password':password,'confirm_password':confirm_password,'is_subscribe_true':is_subscribe_true,'redirect':'','token':''}).then(function(data) {
                    if(!data.is_success){
